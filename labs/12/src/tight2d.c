...
int main(void)
{
    ...
    for(int i = 0; i < N; i++){
        ...
        // vertical neighbors
        if(y > 0) matrix[up + i * N] = -t; // row i, column up
        if(y < Ly - 1) matrix[down + i * N] = -t;

        // horizontal neighbors
        if(x > 0) matrix[left + i * N] = -t;
        if(x < Lx - 1) matrix[right + i * N] = -t;
    }
    ...
    const char *eigenvalue_file = "labs/12/data/eigenvalues2d.dat";
